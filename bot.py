...
from aiogram.types import Message, MessageEntity, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo, MenuButtonWebApp
...
def save_custom_emoji(key, emoji_id):
    with closing(db()) as con:
        con.execute(
            "INSERT OR REPLACE INTO emoji_settings(key, custom_emoji_id) VALUES(?,?)",
            (key, emoji_id),
        )
        con.commit()
...
MESSAGE_KEYS = tuple(DEFAULT_MESSAGES.keys())

def get_bot_message(key):
    with closing(db()) as con:
        row = con.execute("SELECT text FROM bot_messages WHERE key=?", (key,)).fetchone()
    return row[0] if row and row[0] else DEFAULT_MESSAGES.get(key, "")

def save_bot_message(key, text):
    with closing(db()) as con:
        con.execute("INSERT OR REPLACE INTO bot_messages(key,text) VALUES(?,?)", (key, text))
        con.commit()

def render_message(key, **values):
    template = get_bot_message(key)
    class Safe(dict):
...
def answer_custom(message, key, **values):
    text = render_message(key, **values)
    emoji = get_custom_emoji(key)
    if emoji and str(emoji).isdigit():
        text = "⬜ " + text
        entities = [MessageEntity(type="custom_emoji", offset=0, length=1, custom_emoji_id=str(emoji))]
        return message.answer(text, entities=entities)
    fallback = CUSTOM_EMOJIS.get(key, "")
    return message.answer((fallback + " " + text).strip())

def user_values(uid, tg_user=None):
    u = get_user(uid)
    return {
        "name": (tg_user.first_name if tg_user else "") or (u[2] if u else ""),
        "username": (tg_user.username if tg_user else "") or (u[1] if u else ""),
        "balance": u[3] if u else 0,
        "total_earned": u[4] if u else 0,
        "referrals": u[6] if u else 0,
...
def ensure_user(tg_user, referrer_id=None):
    ...
    with closing(db()) as con:
        ...
        con.commit()
    if referral_created and valid_ref and tg_bot:
        try:
            asyncio.create_task(tg_bot.send_message(valid_ref, f"🎉 Referral Success\n\nUser {uid} started from your referral link.\n💰 Balance added: ₹{REFERRAL_REWARD:g}"))
        except Exception:
            pass
...
async def force_join_status(user_id: int):
    ...
    for cid, chat_id, title, invite_link, active in channels:
        try:
            ...
            if not ok:
                missing.append({'id':cid,'chat_id':chat_id,'title':title,'invite_link':invite_link})
        except Exception:
            missing.append({'id':cid,'chat_id':chat_id,'title':title,'invite_link':invite_link})
    return missing
...
async def enforce_force_join(message: Message):
    ...
    rows=[]
    for c in missing:
        url=c['invite_link'] or (('https://t.me/'+c['chat_id'].lstrip('@')) if str(c['chat_id']).startswith('@') else '')
        if url:
            rows.append([InlineKeyboardButton(text=f"📢 Join {c['title']}", url=url)])
    rows.append([InlineKeyboardButton(text='🔄 I’ve Joined — Check Again', callback_data='force_join_check')])
    await message.answer('🔒 <b>Join Required Channels</b>\n\nPlease join all required channels, then tap <b>I’ve Joined — Check Again</b>.', reply_markup=InlineKeyboardMarkup(inline_keyboard=rows), parse_mode='HTML')
    return False
...
async def admin_force_join_add(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    chat_id=str(payload.get('chat_id') or payload.get('username') or '').strip()
    title=str(payload.get('title') or chat_id).strip()
    invite=str(payload.get('invite_link') or '').strip() or None
    if not chat_id: raise HTTPException(400,'Channel username or chat ID is required')
    if chat_id.startswith('https://t.me/'):
        chat_id='@'+chat_id.rsplit('/',1)[-1]
    if not chat_id.startswith('@') and not chat_id.lstrip('-').isdigit():
        chat_id='@'+chat_id
    if not invite and chat_id.startswith('@'): invite='https://t.me/'+chat_id[1:]
    if tg_bot:
        try:
            await tg_bot.get_chat(chat_id)
        except Exception as e:
            raise HTTPException(400,'Bot cannot access this channel. Add the bot as administrator and check the channel username/chat ID.')
    with closing(db()) as con:
...
async def admin_force_join_toggle(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    with closing(db()) as con:
        con.execute('UPDATE force_join_channels SET active=? WHERE id=?',(1 if bool(payload.get('active',True)) else 0,int(payload.get('id',0))))
        con.commit()
    return {'ok':True}
...
async def admin_force_join_delete(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    with closing(db()) as con:
        con.execute('DELETE FROM force_join_channels WHERE id=?',(int(payload.get('id',0)),)); con.commit()
    return {'ok':True}

@app.get("/api/force-join/channels")
async def force_join_channels_api(x_telegram_init_data: str | None = Header(default=None)):
    verify_init_data(x_telegram_init_data)
    rows=await get_force_join_channels()
    return {'channels':[{'id':r[0],'chat_id':r[1],'title':r[2],'invite_link':r[3],'username':r[1].lstrip('@'),'enabled':bool(r[4])} for r in rows]}

@app.post("/api/force-join/verify")
async def force_join_verify_api(x_telegram_init_data: str | None = Header(default=None)):
    u=verify_init_data(x_telegram_init_data)
    missing=await force_join_status(int(u['id']))
    return {'ok':not missing,'joined':not missing,'missing':missing}

@app.get("/api/me/balance")
async def me_balance_alias(x_telegram_init_data: str | None = Header(default=None)):
    return await me(x_telegram_init_data)

@app.post("/api/gift/claim")
async def gift_claim_alias(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    return await gift(payload, x_telegram_init_data)

@app.put("/api/admin/force-join/channels")
async def admin_force_join_channels_alias(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    channels=payload.get('channels') or []
    with closing(db()) as con:
        con.execute('UPDATE force_join_channels SET active=0')
        for c in channels:
            chat_id=str(c.get('chat_id') or c.get('username') or '').strip()
            if not chat_id: continue
            if not chat_id.startswith('@') and not chat_id.lstrip('-').isdigit(): chat_id='@'+chat_id
            title=str(c.get('title') or chat_id)
            invite=str(c.get('invite_link') or ('https://t.me/'+chat_id[1:] if chat_id.startswith('@') else '')) or None
            con.execute("INSERT INTO force_join_channels(chat_id,title,invite_link,active,created_at) VALUES(?,?,?,?,?) ON CONFLICT(chat_id) DO UPDATE SET title=excluded.title,invite_link=excluded.invite_link,active=excluded.active",(chat_id,title,invite,1 if c.get('enabled',True) else 0,int(time.time())))
        con.commit()
    return {'ok':True}

@app.post("/api/notifications/event")
async def notifications_event(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    u=verify_init_data(x_telegram_init_data); uid=int(u['id']); typ=str(payload.get('type','')).strip()
    labels={'balance_added':'💰 Balance Added','referral_success':'🎉 Referral Success','withdrawal_processing':'⏳ Withdrawal Processing','withdrawal_paid':'✅ Withdrawal Paid'}
    if typ in labels and tg_bot:
        amount=payload.get('amount') or payload.get('balance') or 0
        try: await tg_bot.send_message(uid, f"{labels[typ]}\n\nAmount: ₹{float(amount):g}")
...
async def admin_messages(x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    return {"messages": [{"key": k, "text": get_bot_message(k)} for k in MESSAGE_KEYS]}

@app.post("/api/admin/messages")
async def admin_save_message(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    key = str(payload.get("key", "")).strip().lower()
    text = str(payload.get("text", ""))
    if key not in MESSAGE_KEYS:
        raise HTTPException(400, "Invalid message key")
    if not text.strip():
        raise HTTPException(400, "Message cannot be empty")
    if len(text) > 4096:
        raise HTTPException(400, "Message is too long (max 4096 characters)")
    save_bot_message(key, text)
...
async def admin_withdrawal(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    with closing(db()) as con:
        ...
        con.commit()
    target_uid=row[0]
    if tg_bot and action == 'processing':
        try: await tg_bot.send_message(target_uid, f"⏳ Withdrawal Processing\n\nWithdrawal #{wid} of ₹{row[1]:g} is now processing.")
        except Exception: pass
    if tg_bot and action == 'reject':
        try: await tg_bot.send_message(target_uid, f"❌ Withdrawal Rejected\n\nWithdrawal #{wid} of ₹{row[1]:g} was rejected and ₹{row[1]:g} was refunded.")
        except Exception: pass
...
async def admin_balance(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    with closing(db()) as con:
        ...
        con.commit()
    if amount > 0 and tg_bot:
        try: await tg_bot.send_message(uid, f"💰 Balance Added\n\n₹{amount:g} has been added to your balance.")
        except Exception: pass
...
@app.get("/api/referral-tracker/{target_user_id}")
async def referral_tracker(target_user_id: int, x_telegram_init_data: str | None = Header(default=None)):
    verify_init_data(x_telegram_init_data)
    with closing(db()) as con:
        row=con.execute("SELECT id,username,first_name,referred_by FROM users WHERE id=?",(target_user_id,)).fetchone()
        if not row:
            return {'found':False,'user_id':target_user_id,'bot_started':False}
        ref=None
        if row[3]: ref=con.execute("SELECT id,username,first_name FROM users WHERE id=?",(row[3],)).fetchone()
    return {'found':True,'user_id':row[0],'username':row[1],'name':row[2],'bot_started':True,'started_from':({'user_id':ref[0],'username':ref[1],'name':ref[2]} if ref else None)}
...
async def send_home(message: Message):
    ...
    text = render_message("welcome", **user_values(message.from_user.id, message.from_user))
...
async def force_join_check(callback: CallbackQuery):
    ...
    if missing:
        rows=[]
        for c in missing:
            if c.get('invite_link'): rows.append([InlineKeyboardButton(text=f"📢 Join {c['title']}",url=c['invite_link'])])
        rows.append([InlineKeyboardButton(text='🔄 I’ve Joined — Check Again',callback_data='force_join_check')])
        await callback.answer('You still need to join all required channels.',show_alert=True)
        try: await callback.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
        except Exception: pass
        return
...
async def balance(message: Message):
    ...
    await answer_custom(message, "balance", **user_values(message.from_user.id, message.from_user))
...
async def refer(message: Message):
    ...
    await answer_custom(message, "refer", **user_values(message.from_user.id, message.from_user))
...
async def payout_method(message: Message):
    ...
    await answer_custom(message, "payout", **user_values(message.from_user.id, message.from_user))
...
async def withdraw_help(message: Message):
    ...
    await answer_custom(message, "withdraw", **user_values(message.from_user.id, message.from_user))
...
async def gift_help(message: Message):
    ...
    await answer_custom(message, "gift", **user_values(message.from_user.id, message.from_user))
...
async def earn_more(message: Message):
    ...
    if not rows:
        return await answer_custom(message, "earn_more", **user_values(message.from_user.id, message.from_user))
    parts=["🤑 Earn More\n"]
