import os, json, hmac, hashlib, time, sqlite3, asyncio, shutil, base64, urllib.request, urllib.error
...
def create_local_backup():
    ...
    dst = os.path.join(folder, f"bot_{stamp}.db")
    shutil.copy2(src, dst)
    return dst

def _gh_config():
    owner=os.getenv("GITHUB_BACKUP_OWNER","").strip()
    repo=os.getenv("GITHUB_BACKUP_REPO","").strip()
    token=os.getenv("GITHUB_BACKUP_TOKEN","").strip()
    branch=os.getenv("GITHUB_BACKUP_BRANCH","main").strip() or "main"
    folder=os.getenv("GITHUB_BACKUP_FOLDER","bot-backups").strip().strip("/")
    if not owner or not repo or not token:
        raise RuntimeError("GitHub backup is not configured")
    return owner,repo,token,branch,folder

def _gh_request(url, method="GET", payload=None):
    _,_,token,_,_= _gh_config()
    headers={"Authorization":f"Bearer {token}","Accept":"application/vnd.github+json",
             "X-GitHub-Api-Version":"2022-11-28","User-Agent":"BotByte-Backup"}
    ...
    try:
        ...
        raise RuntimeError(f"GitHub error {e.code}: {e.read().decode(errors='ignore')[:300]}")

def github_backup():
    src=_db_path()
    if not os.path.exists(src): raise FileNotFoundError("bot.db not found")
    owner,repo,_,branch,folder=_gh_config()
    stamp=time.strftime("%Y%m%d_%H%M%S")
    name=f"{folder}/bot_{stamp}.db"
    with open(src,"rb") as f: content=base64.b64encode(f.read()).decode()
    data=_gh_request(f"https://api.github.com/repos/{owner}/{repo}/contents/{name}","PUT",
                     {"message":f"BotByte DB backup {stamp}","content":content,"branch":branch})
    return data.get("content",{}).get("name",name)

def github_list_backups():
    owner,repo,_,branch,folder=_gh_config()
    data=_gh_request(f"https://api.github.com/repos/{owner}/{repo}/contents/{folder}?ref={branch}")
    return sorted([{"name":x.get("name","")} for x in data if x.get("type")=="file" and x.get("name","").endswith(".db")],
                  key=lambda x:x["name"],reverse=True)

def github_restore(filename):
    owner,repo,_,branch,folder=_gh_config()
    safe=os.path.basename(filename)
    if safe!=filename or not safe.endswith(".db"): raise RuntimeError("Invalid backup filename")
    data=_gh_request(f"https://api.github.com/repos/{owner}/{repo}/contents/{folder}/{safe}?ref={branch}")
    raw=base64.b64decode(data.get("content",""))
    target=_db_path()
    if os.path.exists(target): shutil.copy2(target,target+".before_restore")
    temp=target+".restore.tmp"
    with open(temp,"wb") as f: f.write(raw)
...
@app.post("/api/admin/backup/local")
async def admin_backup_local(x_telegram_init_data: str | None = Header(default=None)):
    u=verify_init_data(x_telegram_init_data)
    if not _is_admin(int(u["id"])): raise HTTPException(403,"Admin only")
    try: return {"ok":True,"name":os.path.basename(create_local_backup())}
    except Exception as e: raise HTTPException(500,str(e))

@app.get("/api/admin/backup/list")
async def admin_backup_list(x_telegram_init_data: str | None = Header(default=None)):
    u=verify_init_data(x_telegram_init_data)
    if not _is_admin(int(u["id"])): raise HTTPException(403,"Admin only")
    try: return {"configured":True,"items":github_list_backups()}
    except Exception as e: return {"configured":False,"items":[],"detail":str(e)}

@app.post("/api/admin/backup/github")
async def admin_backup_github(x_telegram_init_data: str | None = Header(default=None)):
    u=verify_init_data(x_telegram_init_data)
    if not _is_admin(int(u["id"])): raise HTTPException(403,"Admin only")
    try: return {"ok":True,"name":github_backup()}
    except Exception as e: raise HTTPException(500,str(e))

@app.post("/api/admin/backup/restore")
async def admin_backup_restore(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    u=verify_init_data(x_telegram_init_data)
    if not _is_admin(int(u["id"])): raise HTTPException(403,"Admin only")
    try: return {"ok":True,"path":github_restore(str(payload.get("filename","")).strip())}
    except Exception as e: raise HTTPException(500,str(e))

async def automatic_backup_loop():
    interval=max(3600,int(os.getenv("BACKUP_INTERVAL_SECONDS","21600")))
    while True:
        await asyncio.sleep(interval)
        if os.getenv("GITHUB_BACKUP_TOKEN","").strip():
            try: github_backup()
            except Exception as e: print("Automatic GitHub backup failed:",e)
...
async def main():
    ...
    await asyncio.gather(dp.start_polling(bot), server.serve(), automatic_backup_loop())
