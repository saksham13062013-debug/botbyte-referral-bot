...
BOT_MESSAGES = {
    ...
    "refer": "💕 Refer & Earn\n\n🎁 Per Invite ₹{reward} UPI Cash!!\n\nInvite Link: {link}\n\n✅ Share your own invite link to earn unlimited easy cash!",
    "payout": "💳 Choose Desired Payment Method From Below 👇\n\nYour Current Method: {method}\nPayout Details: {value}",
    ...
    "invitation_contest": "🏆 Invitation Contest\n\nContest details will be announced by the admin.",
}
...
def set_setting(key, value):
    with closing(db()) as con:
        con.execute("INSERT OR REPLACE INTO bot_settings(key,value) VALUES(?,?)", (key, value))
        con.commit()

def render_message(key, **values):
    text = get_setting(key, BOT_MESSAGES.get(key, ""))
    ...
    with closing(db()) as con:
        ...
        if row:
            ...
            if valid_ref:
                ...
                credited_referrer = valid_ref
        con.commit()
    return credited_referrer
...
async def admin_page():
    return FileResponse("admin.html", headers={"Cache-Control":"no-store, no-cache, must-revalidate, max-age=0"})
...
async def admin_messages(x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    keys = list(BOT_MESSAGES.keys())
    return {"messages": {k: get_setting(k, BOT_MESSAGES[k]) for k in keys}}

@app.post("/api/admin/messages")
async def admin_save_messages(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    allowed = set(BOT_MESSAGES.keys())
    data = payload.get("messages", payload)
    if not isinstance(data, dict):
        raise HTTPException(400, "Invalid messages payload")
    saved = 0
...
async def send_home(message: Message):
    ...
    text = render_message("welcome", reward=f"{REFERRAL_REWARD:g}", min=f"{MIN_WITHDRAWAL:g}")
...
async def start(message: Message):
    ...
    if len(parts) == 2 and parts[1].isdigit():
        ref = int(parts[1])
    credited_referrer = ensure_user(message.from_user, ref)
    await send_home(message)
    if credited_referrer and tg_bot:
        try:
            referrer = get_user(credited_referrer)
            if referrer:
                await tg_bot.send_message(
                    credited_referrer,
                    f"🎉 Refer Success!\n\n💰 ₹{REFERRAL_REWARD:g} has been added to your balance.\n💰 Current Balance: ₹{referrer[3]:.2f}\n👥 Total Referrals: {referrer[6]}\n\n🚀 Keep referring & earning more!"
                )
        except Exception:
            pass
...
async def setemoji_start(message: Message):
    ...
    await message.answer(
        ...
        "Use /setemoji <button> and then send the Premium/custom emoji alone.\n\n"
        "balance • payout • gift • refer • withdraw • earn_more • leaderboard"
    )
...
async def adminpanel(message: Message):
    ...
    await message.answer("🛠 Admin Panel\n\nCustomize welcome/button messages, balance, leaderboard, withdrawals, Earn More, users and emojis from here.", reply_markup=kb)

@dp.message(F.text == "🎁 Balance")
async def balance(message: Message):
    ensure_user(message.from_user)
    u = get_user(message.from_user.id)
    await message.answer(render_message("balance", balance=f"{u[3]:.2f}", earned=f"{u[4]:.2f}", referrals=u[6]))
...
async def refer(message: Message):
    ...
    kb = ReplyKeyboardBuilder()
    kb.button(text="👥 My Invites")
    kb.button(text="🏆 Invitation Contest")
    kb.button(text="📊 Refer Tracker")
    kb.button(text="🔙 Dashboard")
    kb.adjust(2,1,1)
    text = render_message("refer", reward=f"{REFERRAL_REWARD:g}", link=link)
    await message.answer(text, reply_markup=kb.as_markup(resize_keyboard=True))

@dp.message(F.text == "👥 My Invites")
async def my_invites(message: Message):
    ensure_user(message.from_user)
    with closing(db()) as con:
        rows = con.execute("SELECT username,first_name,created_at FROM users WHERE referred_by=? ORDER BY created_at DESC LIMIT 50", (message.from_user.id,)).fetchall()
    if not rows:
        return await message.answer("👥 My Invites\n\nNo successful invites yet. Share your referral link to start earning!")
    lines = []
    for i,(username,first_name,_) in enumerate(rows,1):
        name = f"@{username}" if username else (first_name or "User")
        lines.append(f"{i}. {name}")
    await message.answer("👥 My Invites\n\n" + "\n".join(lines))

@dp.message(F.text == "🏆 Invitation Contest")
async def invitation_contest(message: Message):
    await message.answer(get_setting("invitation_contest", BOT_MESSAGES["invitation_contest"]))

@dp.message(F.text == "📊 Refer Tracker")
async def refer_tracker(message: Message):
    u = get_user(message.from_user.id)
    await message.answer(
        f"📊 Refer Tracker\n\n👥 Successful Referrals: {u[6]}\n"
        f"💰 Referral Earnings: ₹{u[4]:.2f}\n🎁 Per Invite: ₹{REFERRAL_REWARD:g}"
    )

@dp.message(F.text == "🔙 Dashboard")
async def dashboard_back(message: Message):
    await send_home(message)
...
async def payout_method(message: Message):
    ensure_user(message.from_user)
    u = get_user(message.from_user.id)
    method = u[7] or "Not set"
    value = u[8] or "Not set"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 UPI", callback_data="payout_upi")],
        [InlineKeyboardButton(text="🏦 Bank Account", callback_data="payout_bank")],
    ])
    await message.answer(render_message("payout", method=method, value=value), reply_markup=kb)

@dp.callback_query(F.data == "payout_upi")
async def payout_upi(callback: CallbackQuery):
    pending_action[callback.from_user.id] = "payout_upi"
    await callback.answer()
    await callback.message.answer("💳 UPI selected.\n\nSend your UPI ID, for example:\n9111336909@upi")

@dp.callback_query(F.data == "payout_bank")
async def payout_bank(callback: CallbackQuery):
    pending_action[callback.from_user.id] = "payout_bank"
    await callback.answer()
    await callback.message.answer("🏦 Bank Account selected.\n\nSend in this format:\nAccountNumber/IFSC")
...
async def bot_leaderboard(message: Message):
    ...
    with closing(db()) as con:
        ...
        """).fetchall()
    if not rows:
        return await message.answer(render_message("leaderboard", leaderboard="No users yet."))
    lines = [f"{i}. {name} — 👥 {refs} referrals — 💸 ₹{wd:.2f} withdrawn" for i,(name,refs,wd) in enumerate(rows,1)]
    await message.answer(render_message("leaderboard", leaderboard="\n".join(lines)))
...
async def gift_help(message: Message):
    ...
    await message.answer(render_message("gift"))
...
async def earn_more(message: Message):
    ...
    if not rows:
        return await message.answer(get_setting("earn_more", BOT_MESSAGES["earn_more"]))
    parts=["🤑 Earn More\n"]
        ...
        if action.startswith("setemoji:"):
            ...
            if action == "payout":
                ...
                if len(parts) != 2 or parts[0].lower() not in ("upi","bank"):
                    return await message.answer("❌ Choose UPI or Bank Account above.")
                method, value = parts[0].upper(), parts[1].strip()
            elif action == "payout_upi":
                method, value = "UPI", text
            else:
                if "/" not in text:
                    return await message.answer("❌ Format: AccountNumber/IFSC")
