...
def force_channels():
    with closing(db()) as con:
        return con.execute("SELECT id,chat_id,title,invite_link,active FROM force_channels WHERE active=1 ORDER BY id").fetchall()

async def user_has_joined_required_channels(uid):
    rows = force_channels()
    if not rows or not tg_bot:
        return True
    for _cid, chat_id, _title, _invite, _active in rows:
        try:
            member = await tg_bot.get_chat_member(chat_id, uid)
            ...
            # If Telegram cannot verify a configured channel, fail closed for safety.
            return False
    return True

async def send_force_join_prompt(target):
    rows = force_channels()
    kb=[]
    for _id, _chat_id, title, invite, _active in rows:
        if invite:
            kb.append([InlineKeyboardButton(text=f"📢 Join {title}", url=invite)])
    kb.append([InlineKeyboardButton(text="✅ I Joined — Check Again", callback_data="force_join_check")])
    await target.answer("🔒 Please join the required channel(s) before using the bot.", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))

async def enforce_force_join(message):
    if not force_channels():
        return True
    ok = await user_has_joined_required_channels(message.from_user.id)
    if not ok:
    ...
    with closing(db()) as con:
        ...
        if row:
            ...
            if valid_ref:
                ...
                try:
                    asyncio.create_task(notify_user(valid_ref, f"🎉 Refer Success!\n\nA new user joined from your referral link.\n💰 Balance added: ₹{REFERRAL_REWARD:g}\n👥 Successful referrals: {con.execute('SELECT referrals FROM users WHERE id=?',(valid_ref,)).fetchone()[0]}"))
                except RuntimeError:
                    pass
        con.commit()
...
async def api_force_join(x_telegram_init_data: str | None = Header(default=None)):
    u = verify_init_data(x_telegram_init_data); uid = int(u["id"])
    rows = force_channels()
    if not rows:
        return {"required": False, "channels": []}
    joined = await user_has_joined_required_channels(uid)
    return {"required": not joined, "channels": [{"id":r[0],"chat_id":r[1],"title":r[2],"invite_link":r[3]} for r in rows]}

@app.get("/api/admin/force-join")
async def admin_force_join_list(x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    with closing(db()) as con:
        rows=con.execute("SELECT id,chat_id,title,invite_link,active FROM force_channels ORDER BY id DESC").fetchall()
    return {"channels":[{"id":r[0],"chat_id":r[1],"title":r[2],"invite_link":r[3],"active":bool(r[4])} for r in rows]}

@app.post("/api/admin/force-join")
async def admin_force_join_save(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    chat_id=str(payload.get("chat_id","")).strip(); title=str(payload.get("title","")).strip() or chat_id; invite=str(payload.get("invite_link","")).strip() or None
    if not chat_id: raise HTTPException(400,"Channel username or chat ID is required")
    with closing(db()) as con:
        cur=con.execute("INSERT INTO force_channels(chat_id,title,invite_link,active,created_at) VALUES(?,?,?,1,?) ON CONFLICT(chat_id) DO UPDATE SET title=excluded.title,invite_link=excluded.invite_link,active=1",(chat_id,title,invite,int(time.time())))
        con.commit()
    return {"ok":True}
...
async def admin_force_join_toggle(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    cid=int(payload.get("id",0)); active=1 if bool(payload.get("active",True)) else 0
    with closing(db()) as con:
        con.execute("UPDATE force_channels SET active=? WHERE id=?",(active,cid)); con.commit()
    return {"ok":True}
...
async def admin_force_join_delete(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    with closing(db()) as con:
        con.execute("DELETE FROM force_channels WHERE id=?",(cid,)); con.commit()
    return {"ok":True}
...
async def admin_save_messages(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    with closing(db()) as con:
        for key, value in messages.items():
            ...
            con.execute("INSERT OR REPLACE INTO message_settings(key,message) VALUES(?,?)", (key,text))
        con.commit()
    return {"ok": True}
...
async def admin_withdrawal(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    if notify_uid:
        if action == "processing":
            await notify_user(notify_uid, f"⏳ Withdrawal Processing\n\nWithdrawal #{wid} of ₹{notify_amount:g} is now being processed.")
        elif action == "paid":
            await notify_user(notify_uid, f"✅ Withdrawal Paid\n\nWithdrawal #{wid} of ₹{notify_amount:g} has been paid successfully. 💸")
    if action == "reject":
        await notify_user(row[0], f"❌ Withdrawal Rejected\n\nWithdrawal #{wid} of ₹{row[1]:g} was rejected.\n💰 The amount has been refunded to your balance.")
...
async def admin_balance(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    with closing(db()) as con:
        ...
        con.commit()
    if amount > 0:
        await notify_user(uid, f"💰 Balance Added\n\n₹{amount:g} has been added to your balance.\n💵 New balance: ₹{new_balance:.2f}")
    else:
        await notify_user(uid, f"💸 Balance Updated\n\n₹{abs(amount):g} was deducted from your balance.\n💵 New balance: ₹{new_balance:.2f}")
...
async def me(x_telegram_init_data: str | None = Header(default=None)):
    ...
    return {
        "user": {"id": uid, "name": u.get("first_name","") or u.get("username","") or "User", "username": u.get("username","")},
...
async def gift(payload: dict, x_telegram_init_data: str | None = Header(default=None)):
    ...
    with closing(db()) as con:
        ...
        con.commit()
    await notify_user(uid, f"🎁 Gift Code Claimed!\n\n💰 ₹{g[0]:g} added to your balance successfully.")
...
async def force_join_check_callback(callback: CallbackQuery):
    if await user_has_joined_required_channels(callback.from_user.id):
        await callback.answer("✅ Verified!", show_alert=False)
        try:
            await callback.message.edit_text("✅ Channel membership verified. You can now use the bot.")
        except Exception:
            pass
...
async def balance(message: Message):
    ...
    await message.answer(get_message("balance", balance=u[3], total_earned=u[4], referrals=u[6]))
...
async def refer(message: Message):
    ...
    await message.answer(get_message("refer", reward=REFERRAL_REWARD, referral_link=link))
...
async def payout_method(message: Message):
    ...
    await message.answer(get_message("payout"))
